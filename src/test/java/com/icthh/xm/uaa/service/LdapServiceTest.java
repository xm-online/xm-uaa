...
public class LdapServiceTest {
    ...
    public void searchByTemplateTest() {
        ...
        Set<Map<String, List<String>>> result = ldapService.searchByTemplate(LDAP_DOMAIN, LDAP_SEARCH_TEMPLATE);
        assertTrue(result.size() > 0);
        assertEquals(4, result.size());

        assertUserUid(result, "test");
        assertUserUid(result, "test-unknown-role");
        assertUserUid(result, "test-user");
        assertUserUid(result, "test-two-roles");
    }

    @Test
    public void searchByTemplateSystemUserTest() {
        TenantProperties.LdapSearchTemplate searchTemplate = tenantProperties.getLdapSearchTemplates().get(0);
        searchTemplate.setQuery("{0}=system");
        tenantProperties.setLdapSearchTemplates(asList(searchTemplate));
        when(tenantPropertiesService.getTenantProps()).thenReturn(tenantProperties);
