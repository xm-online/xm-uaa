...
public final class TestUtil {
    ...
    /**
     * A matcher that tests that the examined number represents the same value - it can be Long, Double,
     * etc - as the reference BigDecimal.
     */
    ...
    public static <T> T createUpdateProxyForBean(T update, T original) {
        ...
        e.setCallback(
            (MethodInterceptor) (_, method, args, _) -> {
                ...
                Object val = update.getClass().getMethod(name, parameterTypes).invoke(update, args);
                if (val == null) {
                    return original.getClass().getMethod(name, parameterTypes).invoke(original, args);
                }
                return val;
            }
        );
